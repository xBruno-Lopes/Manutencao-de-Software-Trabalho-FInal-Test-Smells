...
public class SparseGradientTest extends ExtendedFieldElementAbstractTest<SparseGradient> {
    ...
    public void testConstant() {
        // Teste refatorado 41 e 42
        double testConstant = 1.0;
        int defaultValue = 0;
        SparseGradient grad = SparseGradient.createConstant(c);
        Assert.assertEquals(testConstant, grad.getValue(), 1.0e-15); // returns the value
        Assert.assertEquals(defaultValue, grad.numVars(), 1.0e-15); // has no variables
    }

    @Test
    public void testVariable() {
        //Teste refatorado 50, 51 e 52
        double testVariable = 1.0;
        int hasOneVariable = 1;
        double derivativeOfOne = 1.0
        int id = 0;
        SparseGradient grad = SparseGradient.createVariable(id, v);
        Assert.assertEquals(testVariable, grad.getValue(), 1.0e-15); // returns the value
        Assert.assertEquals(hasOneVariable, grad.numVars(), 1.0e-15); // has one variable
        Assert.assertEquals(derivativeOfOne, grad.getDerivative(id), 1.0e-15); // derivative wr.t itself is 1
    }

...
public class FunctionUtilsTest {
    ...
    @Test
    public void testAdd() {
        //Teste refatorado 117, 105, 104, 103
        UnivariateFunction id = new Identity();
        UnivariateFunction c = new Constant(4);
        UnivariateFunction m = new Minus();
        UnivariateFunction inv = new Inverse();
        ...
        Assert.assertEquals(- 1 / (a * a) -1 + JdkMath.cos(a), FunctionUtils.add(inv, m, c, sin).value(
        new DerivativeStructure(1, 1, 0, a)).getPartialDerivative(1),EPS);
    }


    @Test
    public void testMultiply() {
        //Teste refatorado 146, 142, 136, 128, 124
        UnivariateDifferentiableFunction c = new Constant(4);
        UnivariateDifferentiableFunction id = new Identity();
    ...
    @Test
    public void testSincCombine() {
        //Teste refatorado 187, 159, 154
        ...
        UnivariateFunction id2 = new Identity();
        UnivariateFunction sinc1 = FunctionUtils.combine(div, sin, id2);
    ...
    @Test
    public void testToDifferentiable() {
        // Teste refatorado 317, 269, 231
        final double a = 1.5;
        final double b = 0.5;
        ...
        final MultivariateFunction f = new MultivariateFunction() {
            @Override
            public double value(final double[] point) {
                return a2 * point[0] + b2 * point[1];
            }
        };
        final MultivariateVectorFunction gradient = new MultivariateVectorFunction() {
            @Override
            public double[] value(final double[] point) {
                return new double[] { a2, b2, 0.0 };
            }
        };
        final MultivariateDifferentiableFunction mdf = FunctionUtils.toDifferentiable(f, gradient);
    ...
    public void testDerivative() {
        // Teste refatorado 385, 384, 383, 382, 381, 380, 347, 346, 345
        final double a = 1.5;
        final double b = 0.5;
        final double c = 0.25;
